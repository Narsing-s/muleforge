...
test("secret scanner ignores placeholders and flags literal credentials", () => {
  ...
  try {
    ...
    const secretValue = ["hardcoded", "password", "123"].join("-");
    fs.writeFileSync(path.join(dir, "unsafe.yaml"), ["password:", JSON.stringify(secretValue)].join(" ") + "\\n");
...
test("event runtime generates valid XML with real newlines and correlation header expression", () => {
  ...
  assert.ok(/x-correlation-id/.test(xml) && /default uuid\(\)/.test(xml));
