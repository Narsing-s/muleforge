...
test("verification accepts structured requirements", () => {
  const { verifyProject } = require("../src/verify");
  const fs = require("node:fs");
  const os = require("node:os");
  const path = require("node:path");
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "muleforge-structured-requirement-"));
  try {
    ...
    fs.writeFileSync(path.join(root, "muleforge.yaml"), [
      "requirements:", "  - id: REQ-001", "    text: Build a customer API", "project:", "  name: demo",
      "api:", "  name: Demo", "  basePath: /api/v1", "operations:", "  - name: get", "    method: GET", "    path: /customers"
    ].join("\n"));
...
test("DataWeave validator catches missing header",()=>{const {validateScript}=require("../src/dataweave-validator");assert.equal(validateScript("output application/json --- payload").valid,false);});\n\ntest("DataWeave validator accepts JSON BAT environment configuration",()=>{const {validateScript}=require("../src/dataweave-validator");const report=validateScript(JSON.stringify({baseUrl:"https://SET_ME",id:"SET_ME"}));assert.equal(report.valid,true);assert.equal(report.type,"bat-config");});
