...
function generateConnectivityConfigs(d) {
  ...
  if (sftp) {
    const host = xmlEscape(sftp.host || prop("sftp.host"));
    const port = sftp.port || prop("sftp.port");
    const working = xmlEscape(sftp.path || prop("sftp.workingDir"));
...
function generateMuleXml(config, d) {
  ...
  const databaseConfig = d.hasDatabase
    ? '\n  <db:config name="Database_Config"><db:generic-connection url="' + (db.url || prop("db.url")) + '" driverClassName="' + (d.hasSnowflake ? "net.snowflake.client.jdbc.SnowflakeDriver" : "") + '" user="' + (db.user || prop("db.user")) + '" password="' + (db.password || prop("db.password")) + '" /></db:config>\n'
    : "";
  ...
  const header = '<?xml version="1.0" encoding="UTF-8"?>\n<mule xmlns="http://www.mulesoft.org/schema/mule/core" xmlns:http="http://www.mulesoft.org/schema/mule/http" xmlns:ee="http://www.mulesoft.org/schema/mule/ee/core" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"' + dbNamespace + " " + namespaces(d) + ' xsi:schemaLocation="' + schemas(d) + '">\n  <http:listener-config name="HTTP_Listener_config"><http:listener-connection host="0.0.0.0" port="' + prop("http.port") + '" /></http:listener-config>\n' + connectivityConfigs + databaseConfig;
...
function documentation(model) {
  ...
  return {
    "solution-design.md": `# Solution Design\n\n## Confirmed requirement\n\n${model.requirement}\n\n## API operations\n${endpoints}\n\n## Document-derived connectivity\n${(model.connectivity || []).map(c => `- **${c.type}**${c.endpoint ? ` — endpoint: ${c.endpoint}` : ""}${c.host ? ` — host: ${c.host}` : ""}${c.path ? ` — path: ${c.path}` : ""}${c.queue ? ` — queue: ${c.queue}` : ""}${c.topic ? ` — topic: ${c.topic}` : ""}${c.schedule ? ` — schedule: ${c.schedule}` : ""}`).join("\n") || "- None explicitly detected"}\n\n## Missing configuration\n${(model.missingConfigurations || []).map(x => `- ${x.item} (${x.connector})`).join("\n") || "- None detected"}\n\n## Conflicts\n${(model.conflicts || []).map(x => `- ${x.message}`).join("\n") || "- None detected"}\n\n> Explicit values from the requirement package are preserved. Secrets are never copied into generated source.\n`,
