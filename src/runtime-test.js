...
async function runtimeTest(directory=".",options={}){
 ...
 while(Date.now()<deadline){try{result.request=await request(url);const expected=Number(options.expectedStatus||200);result.ready=result.request.status===expected;break;}catch{await wait(1000);}}
