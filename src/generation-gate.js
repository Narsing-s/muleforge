...
const { validateProtocolCapability } = require("./protocol-capability");
...
function runGenerationGate(configFile, options = {}) {
  ...
  const protocol = validateProtocolCapability(config);
  ...
  const checks = [
    ...
    { id: "protocol-capability", passed: protocol.valid, detail: protocol }
  ];
  ...
  const lines = [
    ...
    "## Protocol capability",
    "",
    `- Requested protocol: ${protocol.protocol || "not explicitly specified"}`,
    `- Capability status: ${protocol.status}`,
    protocol.requiredNextStep ? `- Required next step: ${protocol.requiredNextStep}` : "- Dedicated generation path is available.",
    "",
    "MuleForge blocks verified generation when an explicitly requested protocol does not have a complete implementation path. It must not silently substitute REST scaffolding for another protocol.",
    "",
