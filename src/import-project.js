...
function extractSemantics(xml){
  ...
  const errorHandlers=[]; for(const m of xml.matchAll(/<(on-error-(?:continue|propagate)|on-error)\b([^>]*)>([\s\S]*?)<\/on-error-(?:continue|propagate)|<on-error-(?:continue|propagate)\b([^>]*)\/>/gi)){const tag=m[0],a=attrs(tag);errorHandlers.push({type:(m[1]||"on-error").toLowerCase(),errorType:a.type||a.errorType||null});}
  const configRefs=[...xml.matchAll(/\bconfig-ref="([^"]+)"/gi)].map(m=>m[1]);
  const globalConfigs=[...xml.matchAll(/<([\w-]+):((?:config|connection)|[\w-]+-(?:config|connection))\b([^>]*)/gi)].map(m=>{const a=attrs(m[0]);return {namespace:m[1].toLowerCase(),element:m[2],name:a.name||null};});
