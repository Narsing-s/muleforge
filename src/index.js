...
function validate(file = "muleforge.yaml") { const verification = verifyProject(file); const contract = validateContract(file); const audit = auditProject(file); printReport(verification); console.log("\nContract gate:"); console.log(JSON.stringify(contract, null, 2)); printAudit(audit); if (!verification.ready || !contract.valid || !audit.ready) process.exitCode = 1; }
