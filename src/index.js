...
const { init: initCicd, validate: validateCicd } = require("./cicd");
const VERSION = "0.3.0";
...
const cicd = program.command("cicd").description("Generate and validate CI/CD configuration");
cicd.command("init [config]").description("Generate GitHub Actions CI and interactive CloudHub 2.0 DEV deployment").action((config = "muleforge.yaml") => initCicd(config));
cicd.command("validate [config]").description("Validate generated CI/CD configuration").action((config = "muleforge.yaml") => { if (!validateCicd(config)) process.exitCode = 1; });
