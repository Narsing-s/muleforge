...
function validate(file = "muleforge.yaml") { const verification = verifyProject(file); const contract = validateContract(file); const model = loadConfig(file); const deployment = validateDeployment(model); const connectors = auditConnectors(file); const audit = auditProject(file); printReport(verification); console.log("\nContract gate:"); console.log(JSON.stringify(contract, null, 2)); console.log("\nDeployment gate:"); console.log(JSON.stringify(deployment, null, 2)); console.log("\nConnector integrity gate:"); console.log(JSON.stringify(connectors, null, 2)); printAudit(audit); if (!verification.ready || !contract.valid || !deployment.valid || !connectors.ready || !audit.ready) process.exitCode = 1; }
